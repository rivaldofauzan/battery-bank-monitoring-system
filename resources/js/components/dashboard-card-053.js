...
const dashboardCard053 = () => {
    const ctx = document.getElementById("dashboard-card-053");
    if (!ctx) return;
    ...
    fetch("/json-temperature-data")
        ...
        .then((result) => {
            ...
            const data = result.map((item) => item.average_temperature);
            ...
            const chartValue = document.getElementById(
                "dashboard-card-053-value"
            );
            const chartDeviation = document.getElementById(
                "dashboard-card-053-deviation"
            );
            ...
            const reload = () => {
                ...
                if (increment + range - 1 < result.data.length) {
                    adddata(
                        result.data[increment + range - 1].average_temperature,
                        result.data[increment + range - 2].average_temperature
                    );
                } else {
                    increment = 0;
                    range = data.length;
                    adddata(
                        result.data[increment + range - 1].average_temperature,
                        result.data[result.data.length - 1].average_temperature
                    );
                }
...
export default dashboardCard053;
