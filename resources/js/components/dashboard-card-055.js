...
const dashboardCard055 = () => {
    const ctx = document.getElementById("dashboard-card-055");
    if (!ctx) return;
    ...
    fetch("/public/json-power-data")
        ...
        .then((result) => {
            ...
            const data = result.map((item) => item.average_power);
            ...
            const chartValue = document.getElementById(
                "dashboard-card-055-value"
            );
            const chartDeviation = document.getElementById(
                "dashboard-card-055-deviation"
            );
            ...
            const adddata = (value = NaN, prev) => {
                ...
                chartValue.innerHTML = `${value.toFixed(2)} mWatt`;
            ...
            const reload = () => {
                ...
                if (increment + range - 1 < result.data.length) {
                    adddata(
                        result.data[increment + range - 1].average_power,
                        result.data[increment + range - 2].average_power
                    );
                } else {
                    increment = 0;
                    range = data.length;
                    adddata(
                        result.data[increment + range - 1].average_power,
                        result.data[result.data.length - 1].average_power
                    );
                }
...
export default dashboardCard055;
