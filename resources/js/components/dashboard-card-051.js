...
const dashboardCard051 = () => {
    const ctx = document.getElementById("dashboard-card-051");
    if (!ctx) return;
    ...
    fetch("/public/json-current-data")
        .then((response) => response.json())
        .then((result) => {
            const labels = result.map((item) => item.timestamp);
            const data = result.map((item) => item.average_current);
            ...
            const chartValue = document.getElementById(
                "dashboard-card-051-value"
            );
            const chartDeviation = document.getElementById(
                "dashboard-card-051-deviation"
            );
            ...
            const adddata = (value = NaN, prev) => {
                ...
                chartValue.innerHTML = `${value.toFixed(2)} mA`;
            ...
            const reload = () => {
                ...
                if (increment + range - 1 < result.data.length) {
                    adddata(
                        result.data[increment + range - 1].average_current,
                        result.data[increment + range - 2].average_current
                    );
                } else {
                    increment = 0;
                    range = data.length;
                    adddata(
                        result.data[increment + range - 1].average_current,
                        result.data[result.data.length - 1].average_current
                    );
                }
...
export default dashboardCard051;
