...
const dashboardCard052 = () => {
    const ctx = document.getElementById("dashboard-card-052");
    if (!ctx) return;
    ...
    fetch("/public/json-humidity-data")
        ...
        .then((result) => {
            ...
            const data = result.map((item) => item.average_humidity);
            ...
            const chartValue = document.getElementById(
                "dashboard-card-052-value"
            );
            const chartDeviation = document.getElementById(
                "dashboard-card-052-deviation"
            );
            ...
            const reload = () => {
                ...
                if (increment + range - 1 < result.data.length) {
                    adddata(
                        result.data[increment + range - 1].average_humidity,
                        result.data[increment + range - 2].average_humidity
                    );
                } else {
                    increment = 0;
                    range = data.length;
                    adddata(
                        result.data[increment + range - 1].average_humidity,
                        result.data[result.data.length - 1].average_humidity
                    );
                }
...
export default dashboardCard052;
