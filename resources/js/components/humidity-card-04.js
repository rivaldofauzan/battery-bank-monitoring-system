...
const humidityCard04 = () => {
    const ctx = document.getElementById("humidity-card-04");
    if (!ctx) return;
    ...
    fetch("/json-humidity4")
        .then((response) => response.json())
        .then((result) => {
            const labels = result.map((item) => item.timestamp);
            const data = result.map((item) => item.humidity);
            ...
            const chartValue = document.getElementById(
                "humidity-card-04-value"
            );
            const chartDeviation = document.getElementById(
                "humidity-card-04-deviation"
            );
            ...
            const reload = () => {
                ...
                if (increment + range - 1 < result.data.length) {
                    adddata(
                        result.data[increment + range - 1].humidity,
                        result.data[increment + range - 2].humidity
                    );
                } else {
                    increment = 0;
                    range = data.length;
                    adddata(
                        result.data[increment + range - 1].humidity,
                        result.data[result.data.length - 1].humidity
                    );
                }
