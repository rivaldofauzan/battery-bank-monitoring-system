...
const formatValueWithV = (value) => {
    return `${value.toFixed(2)}°`; // format value to two decimal places with " V" suffix
};
...
const temperatureCard08 = () => {
    const ctx = document.getElementById("temperature-card-08");
    if (!ctx) return;
    ...
    fetch("/json-temperature8")
        .then((response) => response.json())
        .then((result) => {
            const labels = result.map((item) => item.timestamp);
            const data = result.map((item) => item.temperature);
            ...
            const chartValue = document.getElementById(
                "temperature-card-08-value"
            );
            const chartDeviation = document.getElementById(
                "temperature-card-08-deviation"
            );
            ...
            const adddata = (value = NaN, prev) => {
                ...
                chartValue.innerHTML = `${value.toFixed(2)}°`;
            ...
            const reload = () => {
                ...
                if (increment + range - 1 < result.data.length) {
                    adddata(
                        result.data[increment + range - 1].temperature,
                        result.data[increment + range - 2].temperature
                    );
                } else {
                    increment = 0;
                    range = data.length;
                    adddata(
                        result.data[increment + range - 1].temperature,
                        result.data[result.data.length - 1].temperature
                    );
                }
...
export default temperatureCard08;
