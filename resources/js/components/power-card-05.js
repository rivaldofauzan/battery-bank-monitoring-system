...
const formatValueWithV = (value) => {
    return `${value.toFixed(2)} mWatt`; // format value to two decimal places with " V" suffix
};
...
const powerCard05 = () => {
    const ctx = document.getElementById("power-card-05");
    if (!ctx) return;
    ...
    fetch("/json-power5")
        ...
        .then((result) => {
            ...
            const data = result.map((item) => item.power);
            ...
            const chartValue = document.getElementById("power-card-05-value");
            const chartDeviation = document.getElementById(
                "power-card-05-deviation"
            );
            ...
            const reload = () => {
                ...
                if (increment + range - 1 < result.data.length) {
                    adddata(
                        result.data[increment + range - 1].power,
                        result.data[increment + range - 2].power
                    );
                } else {
                    increment = 0;
                    range = data.length;
                    adddata(
                        result.data[increment + range - 1].power,
                        result.data[result.data.length - 1].power
                    );
                }
