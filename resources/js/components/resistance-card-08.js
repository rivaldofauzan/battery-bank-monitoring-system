...
const resistanceCard08 = () => {
    const ctx = document.getElementById("resistance-card-08");
    if (!ctx) return;
    ...
    fetch("/json-resistance8")
        .then((response) => response.json())
        .then((result) => {
            const labels = result.map((item) => item.timestamp);
            const data = result.map((item) => item.resistance);
            ...
            const chartValue = document.getElementById(
                "resistance-card-08-value"
            );
            const chartDeviation = document.getElementById(
                "resistance-card-08-deviation"
            );
            ...
            const reload = () => {
                ...
                if (increment + range - 1 < result.data.length) {
                    adddata(
                        result.data[increment + range - 1].resistance,
                        result.data[increment + range - 2].resistance
                    );
                } else {
                    increment = 0;
                    range = data.length;
                    adddata(
                        result.data[increment + range - 1].resistance,
                        result.data[result.data.length - 1].resistance
                    );
                }
...
export default resistanceCard08;
