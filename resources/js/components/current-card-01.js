...
const formatValueWithV = (value) => {
    return `${value.toFixed(2)} mA`; // format value to two decimal places with " V" suffix
};
...
const currentCard01 = () => {
    const ctx = document.getElementById("current-card-01");
    if (!ctx) return;
    ...
    fetch("/json-current1")
        ...
        .then((result) => {
            ...
            const data = result.map((item) => item.current);
            ...
            const chartValue = document.getElementById("current-card-01-value");
            const chartDeviation = document.getElementById(
                "current-card-01-deviation"
            );
            ...
            const reload = () => {
                ...
                if (increment + range - 1 < result.data.length) {
                    adddata(
                        result.data[increment + range - 1].current,
                        result.data[increment + range - 2].current
                    );
                } else {
                    increment = 0;
                    range = data.length;
                    adddata(
                        result.data[increment + range - 1].current,
                        result.data[result.data.length - 1].current
                    );
                }
