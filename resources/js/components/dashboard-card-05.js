...
const dashboardCard05 = () => {
    ...
    fetch("/json-voltage-data")
        ...
        .then((result) => {
            ...
            const data = result.map((item) => item.average_voltage);
            ...
            const reload = () => {
                ...
                if (increment + range - 1 < result.data.length) {
                    adddata(
                        result.data[increment + range - 1].average_voltage,
                        result.data[increment + range - 2].average_voltage
                    );
                } else {
                    increment = 0;
                    range = data.length;
                    adddata(
                        result.data[increment + range - 1].average_voltage,
                        result.data[result.data.length - 1].average_voltage
                    );
                }
